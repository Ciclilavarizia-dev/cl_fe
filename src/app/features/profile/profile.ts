...
export class Profile implements OnInit {
  ...
  // Aggiunto nuova logice per controllare se l'utente è loggato o meno
  isLogged = false;
  ...
  loadProfile() {
    this.customerHttp.getCustomerProfile().subscribe({
      next: (dto) => {
        ...
        const addresses = dto.addresses.map(
          (a) => new AddressCustomer(a.addressId, a.addressLine1, a.city, a.postalCode)
        );
